...
class LatentVariation(nn.Module):
    def __init__(self, hidden_size, z_hidden_size):
        super(LatentVariation, self).__init__()
        self.z_hidden_size = z_hidden_size

        self.context_to_mean = nn.Linear(hidden_size, z_hidden_size)
        self.context_to_logvar = nn.Linear(hidden_size, z_hidden_size)

    def forward(self, q, a):
        [x, batch_size, _] = q.size()

        context = torch.cat((q, a), 2)
        mean = self.context_to_mean(context)
        logvar = self.context_to_logvar(context)
        std = torch.exp(0.5 * logvar)

        z = Variable(torch.randn([x, batch_size, self.z_hidden_size]))
        z = z.to(device)
        z = z * std + mean  # [4 x batch_size x z_hidden_size]
