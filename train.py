...
def train(input_variable, lengths, target_variable, target_lengths, mask, max_target_len, encoder, hidvar, decoder,
    ...
    _, response_encoder_hidden = encoder(target_variable, target_lengths)

    if hidvar:
        z, mean, logvar = hidvar(encoder_hidden, response_encoder_hidden)
    ...
    if use_teacher_forcing:
        for t in range(max_target_len):
            ...
            mask_loss, nTotal = maskNLLLoss(decoder_output, target_variable[t], mask[t]) # import maskNLLLoss
...
def trainIters(corpus, reverse, n_iteration, learning_rate, batch_size, v, n_layers, hidden_size, print_every,
               save_every, clip, dropout, loadFilename=None, attn_model='dot', decoder_learning_ratio=5.0):
    ...
    if v:
        ...
        decoder = DecoderRNN(attn_model, embedding_decoder, hidden_size * 2, voc.num_words, n_layers, dropout)
        hidvar = LatentVariation(2 * hidden_size, hidden_size)
    else:
    ...
    if loadFilename:
        ...
        if v:
            hidvar_optimizer.load_state_dict(checkpoint['hv_opt'])
    ...
    for iteration in tqdm(range(start_iteration, n_iteration + 1)):
        ...
        loss = train(input_variable, lengths, target_variable, target_lengths, mask, max_target_len, encoder,
                     decoder, hidvar, embedding, encoder_optimizer, decoder_optimizer, batch_size)
        print_loss += loss
        ...
        if (iteration % save_every == 0):
            ...
            torch.save({
                ...
                'hv': hidvar.state_dict(),
                'en_opt': encoder_optimizer.state_dict(),
                'de_opt': decoder_optimizer.state_dict(),
                'hv_opt': hidvar_optimizer.state_dict(),
                'loss': loss,
                'plt': perplexity
